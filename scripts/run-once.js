...
const { openBrowser, createDraftCampaign } = require('../src/braze');
...
(async () => {
  ...
  const needsBrowser = campaigns.some((c) => segmentTemplates[c.segment]);
  const context = needsBrowser ? await openBrowser() : null;
  ...
  try {
    for (const campaign of campaigns) {
      ...
      try {
        const brazeUrl = await createDraftCampaign(context, { ...campaign, templateCampaignId });
