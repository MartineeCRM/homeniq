...
async function createDraftCampaign(context, campaignData) {
  if (campaignData.existingBrazeUrl) {
    console.log('[braze] 기존 복제본 재사용:', campaignData.existingBrazeUrl);
  } else if (campaignData.status === '세팅 실패') {
    console.log('[braze] 세팅 실패 재시도 — 복제 건너뜀, Draft 검색');
  } else {
    await duplicateViaApi(campaignData.campaignName, campaignData.templateCampaignId);
    await new Promise((r) => setTimeout(r, 5000));
  }

  const url = await editCampaign(context, campaignData);
  return url;
}
...
async function editCampaign(context, campaignData) {
  ...
  try {
    ...
    if (!page.url().startsWith(campaignsUrl)) {
      await page.waitForTimeout(3000);
      await page.goto(campaignsUrl, { waitUntil: 'domcontentloaded', timeout: 60000 });
    }

    console.log('[braze] 현재 URL:', page.url());
    // Draft 필터
...
async function handleLoginIfNeeded(page, returnUrl) {
  ...
  if (await emailInput.isVisible({ timeout: 5000 }).catch(() => false)) {
    await emailInput.fill(process.env.BRAZE_EMAIL);
...
module.exports = { openBrowser, createDraftCampaign };
